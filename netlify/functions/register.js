...
// Saves new member to Supabase + sends notification
// ══════════════════════════════════════════
...
exports.handler = async (event) => {
  ...
  if (referred_by) {
    ...
    if (referrer && referrer.length > 0) {
      startPoints = 50; // Bonus for new member
      // Update referrer points (+75)
  ...
  try {
    ...
    // Do not fail registration due to notification error
  }
...
async function sendWelcomeEmail(email, name, refCode, points) {
  ...
  await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to: [email],
      subject: `🎉 Welcome to Seranova, ${name.split(' ')[0]}!`,
...
<body style="margin:0;padding:0;background:#f5f5f5;font-family:'Segoe UI',Arial,sans-serif">
  <div style="max-width:520px;margin:32px auto;background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,.08)">
    ...
    <div style="padding:32px;text-align:center">
      ...
      <a href="https://seranova.world" style="display:inline-block;background:linear-gradient(135deg,#c8a84b,#b8923c);color:#fff;text-decoration:none;padding:14px 32px;border-radius:10px;font-weight:600;font-size:14px">Join the Community →</a>
    </div>
